    ...
    const getNotes = async () => {
      try {
        let logs = await axios({
          ...
          url: "https://notesxdapi.herokuapp.com/notes/allnotes",
        });
          ...
          {result?.map((el: any, index: any) => (
            <div className={classes.section_box} key={index}>
              ...
              <div className={classes.section_info}>
                ...
                >{`${months[parseInt(el.createdAt.split("-")[1]) - 1]
                  .substring(0, 3)
                  .toUpperCase()} ${el.createdAt
                  .split("-")[2]
                  .substring(0, 2)}`}</p>
                <p
                  style={{
                ...
                >{`${Math.ceil(
                  el.body.trim().split(/\s+/).length / wpm
                )} ${x}`}</p>
              </div>
            </div>
          ))}
