...
const SignUpForm = () => {
  ...
  async function signUpUser(event: SyntheticEvent) {
    ...
    if (!passwordMatch) {
      setWarningMsg(
        "Password do not match please check your password entry and try again!"
      );
      setShowWarning(true);
    } else {
      ...
      await axios
        .post("https://notesxdapi.herokuapp.com/users/signup", details)
        .then((response) => {
          // alert("success!")
        ...
        .catch((err) => {
          ...
          console.log('signed up', err.response.data.msg)
          setShowWarning(true);
