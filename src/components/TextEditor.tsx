...
function TextEditor() {
  ...
  const blocksFromHTML = htmlToDraft(
    '<p><span style="color: #8b8989;"><em>You can write your notes here</em></span></p>'
  );
  const [bod, setBod] = useState(
    '<p><span style="color: #949291;"><em>You can write your notes here</em></span></p>'
  );
    ...
    const getNote = async (inp: string) => {
      ...
      try {
        result = await axios({
          ...
          url: `https://notesxdapi.herokuapp.com/notes/${inp}`,
        });
  ...
  const handleSubmit = async () => {
    ...
    try {
      result = await axios({
        ...
        data: { body: markup },
        // withCredentials : true,
        url: `https://notesxdapi.herokuapp.com/notes/editnote/${onEdit}`,
      });
      console.log(result);
